...
//			WaylandCraft.LOGGER.info("RESIZE " + resizeWidth + ", " + resizeHeight + " [" + resizeInitialWidth + ", " + resizeInitialHeight + "]");
	...
	private void prepareToplevel(WLCToplevel toplevel) {
		...
		if(!toplevel.fullscreen || !captureModeEnabled) {
			// Center content geometry only (no SSD outer-frame expansion).
			x = leftMargin * guiScale + Math.max(0, areaWidth * guiScale / 2 - toplevel.geometry.width() / 2f);
			y = topMargin * guiScale + Math.max(0, areaHeight * guiScale / 2 - toplevel.geometry.height() / 2f);
		}
		else {
			x = 0;
			y = 0;
